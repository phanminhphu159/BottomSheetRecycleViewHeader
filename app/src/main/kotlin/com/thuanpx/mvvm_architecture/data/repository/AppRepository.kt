...
import com.thuanpx.mvvm_architecture.data.remote.datasource.ItemDataSource
...
interface AppRepository {
    ...
    fun fetchItem(isLoading: MutableLiveData<Boolean>): Flow<PagingData<Item>>
...
class DefaultAppRepository @Inject constructor(
    ...
    override fun fetchItem(isLoading: MutableLiveData<Boolean>): Flow<PagingData<Item>> {
        return Pager(
            config = PagingConfig(6, enablePlaceholders = false),
            pagingSourceFactory = { ItemDataSource(apiService, isLoading) }
        ).flow
    }
